...
`;
  ...
  // We need to inject `dialogMarkup` right before the last `</div>` of the `if (view === "dashboard")` block
  const beforeAnchor = code.substring(0, idx);
  
  // The last `</div>` should be around here:
  //         </div>
  //       </div>
  //     );
  //   }
  // Let's find `      );\n    }\n` or something similar at the end of `beforeAnchor`
  const lastReturn = beforeAnchor.lastIndexOf('      );');
  if (lastReturn !== -1) {
    const lastDiv = beforeAnchor.lastIndexOf('</div>', lastReturn);
    if (lastDiv !== -1) {
      code = code.substring(0, lastDiv) + dialogMarkup + code.substring(lastDiv);
      fs.writeFileSync('src/routes/_authenticated/espetaculos.tsx', code);
      console.log('Dialog injected inside dashboard view!');
    } else {
      console.log('last div not found');
    }
  } else {
    console.log('last return not found');
  }
} else {
