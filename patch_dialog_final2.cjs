...
const anchor = '// VIEW: WIZARD';

const idx = code.indexOf(anchor);

if (idx !== -1) {
  // We need to find the `</div>\n    );\n  }\n` before this.
  const beforeSection = code.substring(0, idx);
  const closingBraceIdx = beforeSection.lastIndexOf('}');
...
`;
    // Replace `    );\n  }` with the dialog and then `    );\n  }`
    
    // Actually, just insert it before `    ); \n  }`
    const lastReturnEnd = beforeSection.lastIndexOf('</div>');
    if (lastReturnEnd !== -1) {
      code = code.substring(0, lastReturnEnd + 6) + '\n' + dialogMarkup + code.substring(lastReturnEnd + 6);
      
      // wait, `dialogMarkup` already contains the `</div>\n    );\n  }`? No, I only wrote `<Dialog...` at the start of dialogMarkup, and `</div>\n    );\n  }` at the end!
      // I should just insert before `  // ---------------------------------------------------------------------------`
      // But wait! If I just insert the Dialog inside the `</div>` that is being returned by `if (view === "dashboard")` it will work perfectly!
      
      let finalCode = fs.readFileSync('src/routes/_authenticated/espetaculos.tsx', 'utf8');
      const divReturnIdx = finalCode.substring(0, idx).lastIndexOf('</div>\n      );\n    }');
      if (divReturnIdx !== -1) {
        // Insert right before `</div>\n      );\n    }`
        // `dialogMarkup` starts with `<Dialog>` and does NOT have the closing `</div>` because we'll just put it inside!
        // Wait, I defined `dialogMarkup` with `</div>\n    );\n  }` at the end! Let's redefine it cleaner!
      }
    }
  }
}
