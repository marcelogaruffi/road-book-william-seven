...
if (normCode.includes(normAnchor)) {
  code = normCode.replace(normAnchor, dialogMarkup);
  fs.writeFileSync('src/routes/_authenticated/espetaculos.tsx', code);
  console.log('Dialog successfully injected!');
} else {
  console.log('Anchor NOT found');
  const excerpt = normCode.substring(normCode.indexOf('// VIEW: WIZARD') - 100, normCode.indexOf('// VIEW: WIZARD') + 100);
  console.log('Context:\\n', excerpt);
}
