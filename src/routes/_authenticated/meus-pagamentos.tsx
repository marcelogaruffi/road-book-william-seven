...
function MeusPagamentosPage() {
  ...
  const aReceber = pagamentos.filter(p => p.status_pagamento !== 'pago');
  const recebidos = pagamentos.filter(p => p.status_pagamento === 'pago');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      {loading ? (
        <div className="py-12 text-center text-slate-500">Carregando pagamentos...</div>
      ) : pagamentos.length === 0 ? (
        <div className="py-12 text-center text-slate-500 bg-white dark:bg-card/50 rounded-2xl border border-slate-200 dark:border-white/10">
          Você ainda não possui cachês ou pagamentos registrados.
        </div>
      ) : (
        <Tabs defaultValue="a_receber" className="w-full">
          <TabsList className="w-full sm:w-auto h-12 bg-white dark:bg-card/40 border shadow-sm rounded-2xl p-1 mb-6 inline-flex">
            <TabsTrigger value="a_receber" className="px-6 h-full rounded-xl data-[state=active]:bg-primary data-[state=active]:text-primary-foreground font-bold">
              A Receber ({aReceber.length})
            </TabsTrigger>
            <TabsTrigger value="recebidos" className="px-6 h-full rounded-xl data-[state=active]:bg-emerald-500 data-[state=active]:text-white font-bold">
              Recebidos ({recebidos.length})
            </TabsTrigger>
          </TabsList>
          
          <TabsContent value="a_receber" className="mt-0 focus-visible:outline-none">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {aReceber.length === 0 ? (
                <div className="col-span-full py-8 text-center text-slate-500">Você não tem pagamentos pendentes no momento.</div>
              ) : (
                aReceber.map(renderCard)
              )}
            </div>
          </TabsContent>
          
          <TabsContent value="recebidos" className="mt-0 focus-visible:outline-none">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recebidos.length === 0 ? (
                <div className="col-span-full py-8 text-center text-slate-500">Você não tem pagamentos recebidos no momento.</div>
              ) : (
                recebidos.map(renderCard)
              )}
            </div>
          </TabsContent>
        </Tabs>
